...
-- products 테이블에 대한 정책
CREATE POLICY "입점회원은 자신의 상점 제품만 관리할 수 있음" ON products
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY "모든 사용자는 제품 정보를 볼 수 있음" ON products
...
CREATE INDEX idx_product_sales_order_id ON product_sales(order_id);
